...
const selectedUserId = computed({
  get: () => store.state.selectedUserId,
  set: (value: number | null) => store.commit('SET_SELECTED_USER_ID', value),
});
const currentPage = computed(() => store.state.currentPage);
const postsPerPage = computed(() => store.state.postsPerPage);
const toasts = computed(() => store.state.toasts);
const userOptions = computed(() => store.getters.userOptions);
...
const handleSave = async (postData: { title: string; body: string; userId: number }) => {
  await store.dispatch('savePost', { postData, editingPost: editingPost.value });
  closeModal();
};
...
const handlePrevPage = () => {
  if (currentPage.value > 1) store.commit('SET_CURRENT_PAGE', currentPage.value - 1);
};

const handleNextPage = () => {
  if (currentPage.value < totalPages.value) store.commit('SET_CURRENT_PAGE', currentPage.value + 1);
};
...
const handleLimitChange = (limit: number) => {
  store.dispatch('setPostsPerPage', limit);
};
