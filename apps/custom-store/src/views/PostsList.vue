  ...
  <div class="space-y-6 pb-8">
    ...
    <div v-if="!loading && !error && paginatedPosts && paginatedPosts.length > 0" class="flex justify-center pt-4">
      <Pagination
        ...
        @prev="handlePrevPage"
        @next="handleNextPage"
        @change="postsStore.setCurrentPage"
        @change-limit="postsStore.setPostsPerPage"
      />
    </div>
...
const selectedUserId = computed({
  get: () => postsStore.selectedUserId,
  set: (value: number | null) => { postsStore.setSelectedUserId(value); },
});
const currentPage = computed(() => postsStore.currentPage);
const postsPerPage = computed(() => postsStore.postsPerPage);
const toasts = computed(() => postsStore.toasts);
const userOptions = computed(() => postsStore.userOptions.value);
const filteredPosts = computed(() => postsStore.filteredPosts.value || []);
const paginatedPosts = computed(() => postsStore.paginatedPosts.value || []);
const totalPages = computed(() => postsStore.totalPages.value);

// Watch searchQuery changes and update store
const debouncedSetSearchQuery = debounce((value: string) => {
  postsStore.setSearchQuery(value);
}, 300);
...
const handleFilter = (value: string | number) => {
  ...
  postsStore.setSelectedUserId(userId);
};
...
const handleSave = async (postData: { title: string; body: string; userId: number }) => {
  await postsStore.savePost(postData, editingPost.value);
  closeModal();
};
...
const handlePrevPage = () => {
  if (currentPage.value > 1) postsStore.setCurrentPage(currentPage.value - 1);
};

const handleNextPage = () => {
  if (currentPage.value < totalPages.value) postsStore.setCurrentPage(currentPage.value + 1);
};

const removeToast = (id: string) => {
  postsStore.removeToast(id);
};
