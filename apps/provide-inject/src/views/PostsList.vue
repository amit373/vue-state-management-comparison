  ...
  <div class="space-y-6 pb-8">
    ...
    <div v-if="!loading && !error && paginatedPosts && paginatedPosts.length > 0" class="flex justify-center pt-4">
      <Pagination
        ...
        :limit="store.postsPerPage.value"
        @prev="handlePrevPage"
        @next="handleNextPage"
...
const searchQuery = computed({
  get: () => store.searchQuery.value,
  set: (value: string) => { store.searchQuery.value = value; },
});
const selectedUserId = computed({
  get: () => store.selectedUserId.value,
  set: (value: number | null) => { store.selectedUserId.value = value; },
});
const currentPage = computed(() => store.currentPage.value);
const toasts = computed(() => store.toasts.value);
const userOptions = computed(() => store.userOptions.value);
const filteredPosts = computed(() => store.filteredPosts.value || []);
const paginatedPosts = computed(() => store.paginatedPosts.value || []);
const totalPages = computed(() => store.totalPages.value);
...
const handleFilter = (value: string | number) => {
  ...
  store.selectedUserId.value = userId;
  store.handleFilter();
};
